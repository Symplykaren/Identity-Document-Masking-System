...
class MyPaddleOCR:
    ...
    def run_ocr(self, img_path: str, roi=None, debug: bool = False):
        self.img_path = img_path
        ocr_text = []

        if roi is not None:
            # If ROI is specified, crop the image to the specified region
            img = cv2.imread(img_path)
            x, y, w, h = roi
            roi_img = img[y:y + h, x:x + w]

            result = self._ocr.ocr(roi_img, cls=False)
            ...
            # If no ROI is specified, perform OCR on the entire image
